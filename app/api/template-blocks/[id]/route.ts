...
import { UpdateTemplateBlockSchema } from "@/lib/modules/template-block/schema";
...
export const GET = withAuth(async (_session, _request: Request, ctx: Ctx) => {
  ...
  const block = await templateBlockService.getById(id);
...
export const PATCH = withAuth(async (_session, request: Request, ctx: Ctx) => {
  ...
  const { id } = await ctx.params;
  const input = await parseJsonBody(request, UpdateTemplateBlockSchema);
  const block = await templateBlockService.update(id, input, {
    actorType: "ADMIN",
...
export const DELETE = withAuth(async (_session, request: Request, ctx: Ctx) => {
  if (!verifyOrigin(request.headers)) throw new ForbiddenError("Forbidden origin");
  const { id } = await ctx.params;
  await templateBlockService.delete(id, {
    actorType: "ADMIN",
    req: { headers: request.headers },
  });
