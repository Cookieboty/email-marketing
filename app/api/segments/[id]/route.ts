...
import { UpdateSegmentSchema } from "@/lib/modules/segment/schema";
...
export const PATCH = withAuth(async (_session, request: Request, ctx: Ctx) => {
  ...
  const { id } = await ctx.params;
  const input = await parseJsonBody(request, UpdateSegmentSchema);
  const segment = await segmentService.update(id, input, {
    actorType: "ADMIN",
