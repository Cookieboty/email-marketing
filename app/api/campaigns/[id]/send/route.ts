...
import { SendCampaignSchema } from "@/lib/modules/campaign/schema";
...
export const POST = withAuth(async (_session, request: Request, ctx: Ctx) => {
  ...
  const input = await parseJsonBody(request, SendCampaignSchema);
  const campaign = await campaignService.send(id, input, {
    actorType: "ADMIN",
